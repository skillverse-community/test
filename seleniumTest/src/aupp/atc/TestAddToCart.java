...
public class TestAddToCart {
	...
	public static void main(String[] args) {
		...
		//Get all the product description
		...
		//Get the placeholder for the products added
		...
		for(int i = 0; i<nameOfAddedProductsInCart.length;i++)
			nameOfAddedProductsInCart[i] = nameCart.get(i).getText();

		//Get all the product decription
		List<WebElement> descCart = driver.findElements(By.className("inventory_item_desc"));
		String [] descOfAddedProductsInCart = new String[descCart.size()];
		for(int i = 0; i<descOfAddedProducts.length;i++)
		...
		//Get the placeholder for the products that are there in cart
		Product[] productsInCart = new Product[elems.size()];
		//Instantiate products and place it on the placeholder
		for(int i = 0; i<elems.size();i++)
		{
			productsInCart[i] = new Product(nameOfAddedProductsInCart[i], descOfAddedProductsInCart[i], Double.parseDouble(pricesOfAddedProductsInCart[i]));
		}
		
		boolean allProductSame = true;
		for(int i = 0; i<productsInCart.length; i++)
		{
			if(!productsInCart[i].equals(addedProducts[i]))
				allProductSame = false;
		}
		
		boolean greenSignal = false;
		//if the cart size and the size of the productShow case are same
		//if product showcase size and cart size are same
